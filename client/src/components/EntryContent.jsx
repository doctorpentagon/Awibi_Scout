...
import { RiskBadge, TimeCritical } from './ui.jsx';
import { BodyRenderer, humanise } from './BodyRenderer.jsx';
import { Calculator } from './Calculator.jsx';
import { VisualPanel } from '../visuals/VisualPanel.jsx';

/**
 * One entry, rendered in full.
 *
 * Extracted so the entry page and the sequential reader show the SAME thing.
 * A reader working through the library must get the whole entry — warnings,
 * body, checklist, cards — not a summary with a link out of the reading flow,
 * because leaving the flow is how a reader loses their place and stops.
 */
export function EntryContent({ entry, linkMap = null, showBackLink = false }) {
  ...
  return (
    <>
      ...
      {entry.warnings?.length > 0 && (
        <section className="callout callout-danger" aria-labelledby={`warn-${entry.id}`}>
          <h2 id={`warn-${entry.id}`} className="callout-title">
            Warnings
          </h2>
